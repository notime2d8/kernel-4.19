...
*********************************************************/
//add by goodix start
static u8 gtp_test_clean_bak_ref(struct i2c_client *client)
{
	u32 ref_len = 0;
	u8 *refp = NULL;
	u8 ret = 0;
...
//	GTP_ERROR("stone ret of clean bak ref is %d",ret);
	kfree(refp);
	return 0;
}
//add by goodix end
static void goodix_ts_work_func(struct work_struct *work)
{
    ...
    if ((finger == 0x00) && (CHIP_TYPE_GT9F == ts->chip_type))     // request arrived
    {
        ...
        case GTP_RQST_BAK_REF:
			//printk("\n xmcdl into GTP_RQST_BAK_REF\n");
				//add by goodix test 0625 begin
				gtp_test_clean_bak_ref(ts->client);
				rqst_buf[2] = GTP_RQST_RESPONDED;
				gtp_i2c_write(ts->client, rqst_buf, 3);
				goto exit_work_func;
				//add by goodix test 0625 end
			/*
...
#if DEBUG_SWITCH
	for(i = 0; i < cfg_info_len[0]; i++)
	{
		if(i%10 == 0)
			printk("\n");
		printk("0x%02X,", send_cfg_buf[0][i]);
	}
#endif
#if GTP_COMPATIBLE_MODE
    if (CHIP_TYPE_GT9F == ts->chip_type)
...
#if DEBUG_SWITCH
    printk("%s(%d) sensor_id: %d\n", __func__, __LINE__, sensor_id);
	printk("%s(%d) send_cfg_buf:\n", __func__, __LINE__);
