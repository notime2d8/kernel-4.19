...
#define HYN_TP_OPEN_TX_DROP_RATIO      15
	...
	//update pos num
  	...
  	for(rx=0;rx<hyn_ts_data->sensor_rx;rx++){		
		...
		if(tx>0){
				...
				low =((*(p_data+1-hyn_ts_data->sensor_rx))<<8)+(*(p_data-hyn_ts_data->sensor_rx));
				high=((*(p_data+1+(hyn_ts_data->sensor_tx-1)*hyn_ts_data->sensor_rx*2))<<8)+(*(p_data+(hyn_ts_data->sensor_tx-1)*hyn_ts_data->sensor_rx*2));
				delta=high-low;
				open_delta_tx_last=delta;	
		}
		if((((open_delta_tx_last*HYN_TP_OPEN_TX_DROP_RATIO/10)<open_delta)
		||((open_delta_tx_last*10/HYN_TP_OPEN_TX_DROP_RATIO)>open_delta))
		&&(tx>0)){
