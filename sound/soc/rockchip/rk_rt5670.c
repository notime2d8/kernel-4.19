...
*/
#ifdef CONFIG_SND_SOC_ES7210
	if (substream->stream == SNDRV_PCM_STREAM_CAPTURE) {// for rx
		//es7210_pcm_startup_extern();
		//es7210_pcm_hw_params_extern();
	}
#endif
	printk("chensq start cpu_dai snd_soc_dai_set_sysclk\n");
	...
	printk("chensq start codec_dai snd_soc_dai_set_sysclk\n");
	ret = snd_soc_dai_set_sysclk(codec_dai, 0, pll_out,
						 SND_SOC_CLOCK_IN);
	printk("chensq finish codec_dai snd_soc_dai_set_sysclk\n");
	if (ret < 0) {
		dev_err(codec_dai->dev, "Can't set codec clock %d\n", ret);
		return ret;
	}
...
static int rockchip_es7210_hifi_hw_params(struct snd_pcm_substream *substream,
	struct snd_pcm_hw_params *params)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	struct snd_soc_dai *codec_dai = rtd->codec_dai;
...
static struct snd_soc_ops rockchip_es7210_hifi_ops = {
	.hw_params = rockchip_es7210_hifi_hw_params,
};
...
static struct snd_soc_dai_link rockchip_rt5670_dai[] = {
...
#ifdef CONFIG_SND_SOC_ES7243
	{
		...
		.stream_name = "es7243 PCM3",
		.codec_dai_name = "es7243-mic-array",
	},
#endif

#ifdef CONFIG_SND_SOC_ES7210
	{
		.name = "es7210-mic-array",
		.stream_name = "es7210 PCM3",
		.codec_dai_name = "ES7210 4CH ADC 0",
		.ops = &rockchip_es7210_hifi_ops,
	},
#endif
